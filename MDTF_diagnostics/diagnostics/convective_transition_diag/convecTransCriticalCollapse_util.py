            ...
            for Tidx in numpy.arange(t_reg_I.shape[1]):
                if t_reg_I[reg, Tidx]:
                    ...
                    if numpy.nonzero(p_gt_pref)[0].size > 0:  # p_gt_pref non-empty
                        ...
                        wr_idx -= (p[reg, wr_idx, Tidx] - PRECIP_REF) / (
                                    p[reg, wr_idx, Tidx] - p[reg, wr_idx - 1, Tidx])
...
def convecTransCriticalCollapse_plot(argsv1, argsv2, argsv3, argsv4, argsv5, argsv6):
    ...
    if p1_obs.size != 0:
        ...
        fig_obs_cts.suptitle('Convective Transition Collapsed Statistics'
                             '(' + OBS + ', ' + RES + '$^{\circ}$)', y=1.02,
                             fontsize=16)  ##Change y=1.04 to 1.02 for Python3.
        ...
        for reg in numpy.arange(NUMBER_OF_REGIONS):
            ...
            for Tidx in numpy.arange(TEMP_MIN_obs, TEMP_MAX_obs + 1):
                if t_reg_I_obs[reg, Tidx]:
                    if BULK_TROPOSPHERIC_TEMPERATURE_MEASURE == 1:
                        ax1.scatter(Q1_obs[reg, Tidx] / Q0_obs[reg, Tidx] - wc_obs[reg, Tidx],
                                    fig_params['f1'][1][1] * 0.98,
            ...
            ax1.text(0.4, 0.95, "Slope=" + "{:.2f}".format(al_obs[reg]), transform=ax1.transAxes, fontsize=12,
            ...
            for Tidx in numpy.arange(TEMP_MIN_obs, TEMP_MAX_obs + 1):
                if t_reg_I_obs[reg, Tidx]:
                    if BULK_TROPOSPHERIC_TEMPERATURE_MEASURE == 1:
                        ax2.scatter(Q1_obs[reg, Tidx] / Q0_obs[reg, Tidx] - wc_obs[reg, Tidx],
                                    fig_params['f2'][1][1] * 0.98,
                                    edgecolor=scatter_colors[(Tidx - TEMP_MIN_obs) % NoC, :] / 2,
                                    facecolor=scatter_colors[(Tidx - TEMP_MIN_obs) % NoC, :],
                                    s=marker_size, clip_on=True, zorder=3, marker="^")
                    elif BULK_TROPOSPHERIC_TEMPERATURE_MEASURE == 2:
            ...
            for Tidx in numpy.arange(TEMP_MIN_obs, TEMP_MAX_obs + 1):
                if t_reg_I_obs[reg, Tidx]:
                    if BULK_TROPOSPHERIC_TEMPERATURE_MEASURE == 1:
                        ax3.scatter(Q1_obs[reg, Tidx] / Q0_obs[reg, Tidx] - wc_obs[reg, Tidx],
                                    fig_params['f3'][1][1] * 0.83,
                                    edgecolor=scatter_colors[(Tidx - TEMP_MIN_obs) % NoC, :] / 2,
                                    facecolor=scatter_colors[(Tidx - TEMP_MIN_obs) % NoC, :],
                                    s=marker_size, clip_on=True, zorder=3, marker="^")
                    elif BULK_TROPOSPHERIC_TEMPERATURE_MEASURE == 2:
    ...
    for reg in numpy.arange(NUMBER_OF_REGIONS):
        ...
        ax4.set_axisbelow(True)
        if reg == 0:
            ax4.text(s='PDF of CWV for Precip.>' + str(PT) + 'mm/hr', x=0.49, y=1.05, transform=ax4.transAxes,
                     fontsize=12,
                     ha='center', va='bottom')
    ...
    for reg in numpy.arange(NUMBER_OF_REGIONS):
        if BULK_TROPOSPHERIC_TEMPERATURE_MEASURE == 1:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                ax2.plot(temp_bin_center, wc[reg, :] / (Q1[reg, :] / Q0[reg, :]), '-',
                         color=scatter_colors[reg_color[reg], :])
                ax2.scatter(temp_bin_center, wc[reg, :] / (Q1[reg, :] / Q0[reg, :]),
                            color=scatter_colors[reg_color[reg], :],
                            s=marker_size, clip_on=True, zorder=3, label=REGION_STR[reg])
        elif BULK_TROPOSPHERIC_TEMPERATURE_MEASURE == 2:
