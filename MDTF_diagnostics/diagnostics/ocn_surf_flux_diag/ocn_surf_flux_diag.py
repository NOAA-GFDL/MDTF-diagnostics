...
year_lim = [float(os.getenv("startdate")), float(os.getenv("enddate"))]
...
(ds_model_bin.to_netcdf(os.getenv("WORK_DIR") + "/model/netCDF/model_binned.nc"))

(ds_stn_bin.to_netcdf(os.getenv("WORK_DIR") + "/obs/netCDF/obs_binned.nc"))
...
cs = ((ds_stn_bin['hfls_count']/ds_stn_bin['hfls_count'].sum()).plot.contour(x='sfcWind',
                                                                             y='del_q',
                                                                             ax=ax1,
                                                                             levels=ratiolevel,
                                                                             colors='w',
...
cs = ((ds_model_bin['hfls_count']/ds_model_bin['hfls_count'].sum()).plot.contour(x='sfcWind',
                                                                                 y='del_q',
                                                                                 ax=ax2,
                                                                                 levels=ratiolevel,
                                                                                 colors='w',
...
biases_conf = (ds_modelbias_bin_conf['hfls_conf_%0.2f'%(stt)].where(ds_modelbias_bin_conf['hfls_conf_%0.2f'%(stt)]
...
# output figures
fig.savefig(
    os.path.join(os.getenv("WORK_DIR"), "model/LHFLXdiag_modelbias_example.png"),
    facecolor="w",
    edgecolor="w",
