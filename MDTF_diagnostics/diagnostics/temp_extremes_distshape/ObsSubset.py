...
#
#   Subsets input observational netcdf data to seasonal temperature distribution moment text files,
#   seasonal shift ratio text files for both sides of the temperature distribution, and seasonal netcdf files
#
...
with open(os.environ["WORK_DIR"] + "/ObsSubset_parameters.json") as outfile:
...
T2Mfile = \
sorted(glob.glob(sub_data["MODEL_OUTPUT_DIR"] + "/" + sub_data["MODEL"] + "*" + sub_data["T2M_VAR"] + ".day.nc"))[0]
...
msk = Region_Mask(sub_data["REGION_MASK_DIR"] + '/' + sub_data["REGION_MASK_FILENAME"], T2Mfile, sub_data["LON_VAR"],
                  sub_data["LAT_VAR"])
...
for seasind in range(len(sub_data["monthsubs"])):
    ...
    seas_mean, seas_std, seas_skew, lon, lat = Seasonal_Moments(T2Mfile, sub_data["LON_VAR"], sub_data["LAT_VAR"],
                                                                sub_data["T2M_VAR"], sub_data["TIME_VAR"],
                                                                sub_data["monthsubs"][seasind], sub_data["yearbeg"],
                                                                sub_data["yearend"], msk)

    # ---- Save out each moment as text file
    numpy.savetxt(sub_data["MODEL_OUTPUT_DIR"] + '/' + sub_data['MODEL'] + '_moments_mean_' + sub_data["monthstrs"][
        seasind] + '.txt', seas_mean)
    numpy.savetxt(sub_data["MODEL_OUTPUT_DIR"] + '/' + sub_data['MODEL'] + '_moments_std_' + sub_data["monthstrs"][
        seasind] + '.txt', seas_std)
    numpy.savetxt(sub_data["MODEL_OUTPUT_DIR"] + '/' + sub_data['MODEL'] + '_moments_skew_' + sub_data["monthstrs"][
        seasind] + '.txt', seas_skew)

    # ======================================================================
    # ---- Calculate two-meter temperature anomaly
    T2Manom_data, lon, lat = Seasonal_Anomalies(T2Mfile, sub_data["LON_VAR"], sub_data["LAT_VAR"], sub_data["T2M_VAR"],
                                                sub_data["TIME_VAR"], sub_data["monthsubs"][seasind],
                                                sub_data["yearbeg"], sub_data["yearend"])
    ...
    for ptileval in sub_data["ptiles"]:
        ...
        numpy.savetxt(sub_data["MODEL_OUTPUT_DIR"] + '/' + sub_data['MODEL'] + '_shiftratio_' + sub_data["monthstrs"][
            seasind] + '_' + str(ptileval) + 'th-ptile.txt', seas_skew)
