...
# -----  region_mask_filename and model_netcdf_filename are string names referring to directory locations of mask
# and model data
...
def Seasonal_Moments(model_netcdf_filename, lon_var, lat_var, field_var, time_var, monthsub, yearbeg, yearend,
                     mask_region):
...
def Moments_Plot(model_netcdf_filename, lon_var, lat, monthstr, cmaps, titles, data, tickrange, var_units, fig_dir,
                 fig_name):
    ...
    for idata in numpy.arange(0, len(cmaps)):
        ...
        p1 = ax.pcolormesh(lon, lat, data_plt, cmap=cmaps[idata], vmin=numpy.min(tickrange[idata]),
                           vmax=numpy.max(tickrange[idata]), linewidth=0, rasterized=True, transform=ax.projection)
        ...
        cax = inset_axes(ax, width="7%", height="100%", loc='right', bbox_to_anchor=(pos_x, 0, 0.3, 1),
