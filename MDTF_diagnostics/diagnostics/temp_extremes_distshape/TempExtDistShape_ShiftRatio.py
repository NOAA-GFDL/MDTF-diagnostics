...
#
#   Computes the shifted underlying-to-Gaussian distribution tail exceedances ratio following Loikith and Neelin (2019),
#   Loikith et al. (2018), Loikith and Neelin (2015), Ruff and Neelin (2012)
#
...
with open(os.environ["WORK_DIR"] + "/TempExtDistShape_ShiftRatio_parameters.json") as outfile:
...
T2Mfile = (
    sorted(glob.glob(ratio_data["MODEL_OUTPUT_DIR"] + "/" + ratio_data["MODEL"] + "*" + ratio_data["T2M_VAR"] +
                     ".day.nc")))[0]

# ======================================================================
# Load & pre-process region mask
# ----  Generate a map of values corresponding to land regions only
msk = Region_Mask(ratio_data["REGION_MASK_DIR"] + '/' + ratio_data["REGION_MASK_FILENAME"], T2Mfile,
                  ratio_data["LON_VAR"], ratio_data["LAT_VAR"])

# ======================================================================
# Calculate seasonal anomalies for two-meter temperature
T2Manom_data, lon, lat = Seasonal_Anomalies(T2Mfile, ratio_data["LON_VAR"], ratio_data["LAT_VAR"],
                                            ratio_data["T2M_VAR"], ratio_data["TIME_VAR"], monthsub,
                                            ratio_data["yearbeg"], ratio_data["yearend"])
...
ShiftRatio_Plot(T2Mfile, ratio_data["LON_VAR"], ratio_data["COLORMAP_DIR"] + '/' + ratio_data["COLORMAP_FILENAME"], lat,
                shiftratio, ratio_data["monthstr"], ratio_data["ptile"], ratio_data["FIG_OUTPUT_DIR"],
                ratio_data["FIG_OUTPUT_FILENAME"])
