...
# -----  region_mask_filename and model_netcdf_filename are string names referring to directory
# locations of mask and model data
...
def ShiftRatio_Calc(ptile, shift, msk, T2Manom_data, lon, lat):
    ...
    # Mask temperature array by cloning 2D lon-lat mask array output from Mask_Region function to size of
    # temperature array in time dimension
    ...
    # size lon-lat; midpoint to match matlab percentile function interpolation scheme
    pthresh = numpy.percentile(masked_T2M_anom, ptile, axis=2,
                               interpolation='midpoint')

    # Compute number of days exceeding pthresh after shift
    # -----  Loop through each grid cell where 'thrshold[iloncell,ilatcell]' is the percentile
    # threshold 'pthresh' of the two-meter temperature anomaly distribution at grid cell
    # defined by its longitude-latitude coordinate
    if ptile < 50:
        exceedances = numpy.array([[len(numpy.where((masked_T2M_anom[iloncell, ilatcell, :] + shift * numpy.std(
            masked_T2M_anom[iloncell, ilatcell, :], ddof=1)) < pthresh[iloncell, ilatcell])[0]) if ~numpy.isnan(
            pthresh[iloncell, ilatcell]) else numpy.nan for ilatcell in numpy.arange(0, len(lat))] for iloncell in
                                   numpy.arange(0, len(lon))])
    elif ptile > 50:
        exceedances = numpy.array([[len(numpy.where((masked_T2M_anom[iloncell, ilatcell, :] + shift * numpy.std(
            masked_T2M_anom[iloncell, ilatcell, :], ddof=1)) > pthresh[iloncell, ilatcell])[0]) if ~numpy.isnan(
            pthresh[iloncell, ilatcell]) else numpy.nan for ilatcell in numpy.arange(0, len(lat))] for iloncell in
    ...
    # -----  Compute 5th & 95th percentiles of random gaussian distribution shift to determine statistical
    # significance of shift ratio
    gauss_exceedances = []
    ...
    # Compute ratio of exceedances from non-Gaussian shift to median (50th percentile) of shifts
    # from randomly generated Gaussian distributions
...
def ShiftRatio_Plot(model_netcdf_filename, lon_var, colormap_file, lat, shiftratio, monthstr, ptile, fig_dir, fig_name):
    ...
    # (and align latitudes with land borders)
    var_netcdf = Dataset(model_netcdf_filename, "r")
    lon = numpy.asarray(var_netcdf.variables[lon_var][:], dtype="float")
    ...
    if ptile < 50:
        p1 = mplt.pcolormesh(lon, lat, numpy.log10(shiftratio), cmap=mycmap, vmin=numpy.log10(0.125),
                             vmax=numpy.log10(2), transform=ax.projection)
    elif ptile > 50:
        p1 = mplt.pcolormesh(lon, lat, numpy.log10(shiftratio), cmap=mycmap, vmin=numpy.log10(0.5),
                             vmax=numpy.log10(2), transform=ax.projection)
    ...
    ax.set_aspect('equal')
    cax2 = inset_axes(ax, width="100%", height="15%", loc='lower center', bbox_to_anchor=(0, -0.05, 1, 0.3),
                      bbox_transform=ax.transAxes, borderpad=0)
    if ptile < 50:
        cbar = mplt.colorbar(p1, cax=cax2, orientation='horizontal',
                             ticks=[numpy.log10(0.125), numpy.log10(0.25), numpy.log10(0.5), numpy.log10(1),
                                    numpy.log10(2)])
        cbar.ax.set_xticklabels(['1/8', '1/4', '1/2', '1', '2'])
    elif ptile > 50:
        cbar = mplt.colorbar(p1, cax=cax2, orientation='horizontal',
                             ticks=[numpy.log10(0.5), numpy.log10(1), numpy.log10(2)])
