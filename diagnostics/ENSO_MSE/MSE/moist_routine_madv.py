...
def moisture_adv(imax, jmax, zmax, lon, lat, plev, hgt, ta, hus, ua, va, rearth, madv1, madv3, undef):
    ...
    madv3 = np.zeros( (imax,jmax, zmax),dtype='float32', order='F')
    madv3[:,:,:] = undef
    madv1 = np.zeros( (imax,jmax),dtype='float32', order='F')
    ...
    for j in range(1, jmax-1):
        for i in range(0, imax):
            ...
            for k in range(0, zmax):
                ...
                if ((dz < undef2) and (qq11 < undef2) and (qq21 < undef2) and (qq12 < undef2) and (qq22  < undef2) and (uu < undef2) and (vv < undef2) and (rho < undef2)):
                    # temperature advection if defined -[u*dt/dx + v*dt/dy]
                    xx = uu * (qq21-qq11)/(dxx) + vv * (qq22-qq12)/(dyy)  
                    # vertical integration:   muliply by density of air to get w/m2
                    # also minus sign to make it advection as defined
                    madv1[i,j] = madv1[i,j] - xx * dz * lh * rho
                    madv3[i,j,k] = -xx * lh
                    ss = ss + 1.0
