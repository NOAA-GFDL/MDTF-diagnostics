...
def setenv(varname: str, varvalue, env_dict: dict, verbose=0):
    ...
    if verbose > 2:
        print("Saving ", varname, " = ", varvalue)
    os.environ[varname] = varvalue
    env_dict[varname] = varvalue
    if verbose > 2:
        print("Check ", varname, " ", env_dict[varname])
...
def check_required_dirs(verbose=3, already_exist=[], create_if_nec=[]):
    ...
    if verbose > 1:
        print(filestr + " starting")
    for dir_in in already_exist + create_if_nec:
        if verbose > 1:
            print("\t looking for required dir: " + dir_in)

        if dir_in in os.environ:  
            dir = os.environ[dir_in]
            if verbose > 2:
                print(" \t found " + dir_in + " = " + dir)
        else:
            if verbose > 2:
                print("envvar " + dir_in + " not defined. Setting to self.")
            dir = dir_in
