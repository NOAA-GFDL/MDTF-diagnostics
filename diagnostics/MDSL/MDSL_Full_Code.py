...
def coastal_tch_calc(reginfo, nreg):# Add columns to hold TCH results
    ...
    tgs_mod_xr, mod_proximity   = get_model_GFDL(ds_model, tgcsvin=tgcsvin)
...
for nreg in np.arange(len(reginfo)):
    model_reg, cnes_reg, dtu_reg = regrid_regions_gfdl(ds_model, 
                                              ds_obs_cnes, 
                                              ds_obs_dtu, 
...
for nreg in np.arange(len(reginfo)):
    ...
    err_, num_points_, cost_func_, tchgrid_ = regional_tch(ds_model, 
                                                           model_reg,
                                                           cnes_reg,
                                                           dtu_reg,
                                                           reginfo.iloc[nreg].south_bound,
...
regrid_mod = xe.Regridder(ds_model, 
                          dtu_mdt_rg, 
                          "conservative_normed", 
                          extrap_method=None, 
                          periodic=True, 
...
mod_mdt_rg = regrid_mod(ds_model.zos) 
