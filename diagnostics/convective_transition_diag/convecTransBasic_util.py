    ...
    for lat_idx in numpy.arange(CWV.shape[1]):
        ...
        if reg > 0 and reg <= NUMBER_OF_REGIONS:
            ...
            for time_idx in numpy.arange(CWV.shape[0]):
                if (temp_idx[time_idx] < NUMBER_TEMP_BIN and temp_idx[time_idx] >= 0 and cwv_idx[
                    time_idx] < NUMBER_CWV_BIN):
...
def convecTransBasic_calcTaveQsatInt(ta_netcdf_filename, TA_VAR, PRES_VAR, MODEL,
    ...
    while time_idx_start < ta_netcdf.variables[TA_VAR].shape[0]:
        ...
        if FLIP_PRES:
            ta = numpy.asarray(
                ta_netcdf.variables[TA_VAR][time_idx_start:time_idx_end, pfull.size - (p_max + 1):pfull.size - p_min,
                numpy.logical_and(lat >= -20.0, lat <= 20.0), :], dtype="float")
            ta = numpy.fliplr(ta)
        else:
            ta = numpy.asarray(ta_netcdf.variables[TA_VAR][time_idx_start:time_idx_end, p_min:p_max + 1,
                               numpy.logical_and(lat >= -20.0, lat <= 20.0), :], dtype="float")
    ...
    if SAVE_TAVE_QSAT_INT == 1:
        ...
        tave_output_filename = PREPROCESSING_OUTPUT_DIR + "/" + ta_netcdf_filename.split('/')[-1].replace(
            "." + TA_VAR + ".", "." + TAVE_VAR + ".")
        ...
        qsat_int_output_filename = PREPROCESSING_OUTPUT_DIR + "/" + ta_netcdf_filename.split('/')[-1].replace(
            "." + TA_VAR + ".", "." + QSAT_INT_VAR + ".")
...
def convecTransBasic_plot(ret, argsv1, argsv2, *argsv3):
    ...
    for reg in numpy.arange(NUMBER_OF_REGIONS):
        ...
        ax4.set_axisbelow(True)
        if reg == 0:
            ax4.text(s='PDF of CWV for Precip.>' + str(PT) + 'mm/h', x=0.49, y=1.05, transform=ax4.transAxes,
                     fontsize=12,
                     ha='center', va='bottom')
