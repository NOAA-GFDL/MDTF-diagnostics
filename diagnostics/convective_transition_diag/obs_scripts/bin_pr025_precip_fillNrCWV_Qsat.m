...
resolution=0.25; % 0.25, 0.50, 1.00, 1.50, etc
...
tmi_dir='/media/yhkuo/REI/TMI_v7r1/';
trmmpr_dir='/media/yhkuo/REI/2A25/rssGridded/noRainType/';
...
bin_output_filename_suffix='_fillNrCWV_Qsat';
...
disp(datestr(date))
	...
	if (~isempty(time)) % if the data is not missing
		...
		qsat_hat=interp3(lat_reanalysis,lon_reanalysis,[0:6:24],...
									...
									lat,lon,time);
		% qsat_binCenter=[16:4.5:106] (mm)
		q=uint16(round( ( qsat_hat-11.5 )/4.5 ));
		%%
		% Binning
...
% 			that_idx=That(idx);
			...
			if ( (cwv_idx>0)&&(cwv_idx<=number_cwv_bins)...
						...
						&&(rain_idx<=PrecMoments))
				BIN(reg,cwv_idx,that_idx,rain_idx)=BIN(reg,cwv_idx,that_idx,rain_idx)+1;
				...
				if (vapor(idx)>0.6*qsat_hat(idx))
					QSH(reg,that_idx,rain_idx,1)=QSH(reg,that_idx,rain_idx,1)+1;
					QSH(reg,that_idx,rain_idx,2)=QSH(reg,that_idx,rain_idx,2)+qsat_hat(idx);
				end
			end
		end %end for idx
		else
...
save([bin_output_dir bin_output_filename],...
			'cwv','qsatp_binCenter','precip_threshold','precip_bdy','precip_binCenter',...%'tave',...
