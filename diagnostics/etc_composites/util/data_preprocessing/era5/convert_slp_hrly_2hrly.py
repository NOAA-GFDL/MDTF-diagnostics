...
out_folder = '/localdrive/drive6/era5/data/tmp_2hrly/'

for fn in glob.glob(os.path.join(folder, 'slp.*.nc')):
  ds = xr.open_dataset(fn)
  ds_sel = ds.isel(time=np.arange(0, len(ds.time), 2))
  fname = fn.split('/')[-1]
  ...
  time = np.arange(0, len(ds_sel.time)*2, 2)
  ...
  out_ds.time.attrs['delta_t'] = '0000-00-00 02:00:00'
