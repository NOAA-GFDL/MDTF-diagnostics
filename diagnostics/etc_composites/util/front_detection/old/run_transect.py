  ...
  for i_hemis in hemis: 
    ...
    for i_type in front_types: 
      ...
      for i_lm in ['land', 'ocean']:
        transect[var_name][i_hemis][i_type][i_lm] = {'sum': np.zeros((len(front_dist_bins_mid), len(height_bins_mid))), 'cnts': np.zeros((len(front_dist_bins_mid), len(height_bins_mid)))}
...
for year in year_list: 
  ...
  front_file = os.path.join(defines.read_folder, f'fronts_{defines.model}_{year}.nc')
  ...
  for t_step in range(1, len(fronts.time)):
    ...
    for i_center, _  in enumerate(centers.lat): 
      ...
      mask_valid_fronts = (dist_grid < 500)
      ...
      model_z = xr.open_dataset(os.path.join(defines.model_data_directory, f'z.{year}.nc'))
      ...
      if (cf_flag):
        ...
        for var_name in defines.transect_var_list:
          ...
          # variable dataset
          model_var = xr.open_dataset(os.path.join(defines.model_data_directory, f'{var_name}.{year}.nc'))
          ...
          if (center['lat'] < 0):
            ...
            hemis = 'NH'
          transect[var_name][hemis][front_type][lm_type]['sum'] += H_sum
          transect[var_name][hemis][front_type][lm_type]['cnts'] += H_cnts
...
for var_name in defines.transect_var_list:
  ...
  for hemis in defines.transect_hemis_list:
    ...
    ftype = 'cf'
    lm_type = 'ocean'
    tmp = (transect[var_name][hemis][ftype][lm_type]['sum']/transect[var_name][hemis][ftype][lm_type]['cnts']).T
    plt.contourf(transect['front_dist'], transect['height'], tmp, levels=levels, extend='both', cmap=cmap);
    plt.colorbar(); 
    plt.title(f'Across {hemis.upper()} - {lm_type.upper()}, {ftype.upper()}: {var_name.upper()}')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(defines.images_folder, f'Transect_{hemis.upper()}_{var_name.upper()}.png'), dpi=300.) 
