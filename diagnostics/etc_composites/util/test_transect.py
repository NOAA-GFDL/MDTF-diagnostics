...
for year in year_list: 
  ...
  for t_step in tqdm(range(1, len(fronts.time)), total=len(fronts.time), desc=f'{year}: '):
    ...
    for i_center, _  in enumerate(centers.lat): 
      ...
      model_z = xr.open_dataset(os.path.join(defines.var_data_directory, f'transect_z.{year}.nc'))
      ...
      if (cf_flag):
        ...
        for var_ind, var_name in enumerate(defines.transect_var_list):
          ...
          # variable dataset
          model_var = xr.open_dataset(os.path.join(defines.var_data_directory, f'transect_{var_name}.{year}.nc'))
