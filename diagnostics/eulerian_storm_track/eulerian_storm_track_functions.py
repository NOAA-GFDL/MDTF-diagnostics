...
Tracker Notes: 
  ...
  4) average std_dev for all years
'''


def six_hrly_to_daily(data, time):
    """
        Data has to be provided as six hourly timesteps, in a numpy array format (time x lon x lat), lon and lat can be
        changed, but keep track of it
        the time variable has to be given in six hourly increments, since the start_year
...
def std_dev(data, time_ind):
    """
        ...
        we will calculate the std_dev for the given time_ind, the time_ind has to be a logical array of the
...
def get_time_ind(start_year, time, season: str = 'djf'):
    """
      Args:
        start_year:
        time:
      ...
      Returns:
         The time index for the given season
    """
...
def old_std_dev(data, start_year, time, time_period: str = 'yearly', season: str = ''):
    """
        ...
        time vaiable has to be specified in days, since start_year [1,2,3,4,5,6,7], default=finds the time starting
        from day 1
