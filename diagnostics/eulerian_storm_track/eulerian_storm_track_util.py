...
def model_std_dev(data, start_year, time, season='djf'):
    """
       ...
       We will calculate the std_dev for the given time_ind, the time_ind has to be a logical array of the size
    ...
    dates_year = []
    for i_time in time:
        temp_time = dt.datetime(start_year, 1, 1) + dt.timedelta(days=float(i_time)-1)
    ...
    eddy_year = []
    for i_year in range(int(os.environ['startdate']), int(os.environ['enddate']) + 1):
        if season == 'djf':
            time_ind = (((dates_year == i_year) & (dates_month == 1)) | ((dates_year == i_year) & (dates_month == 2))
                        | ((dates_year == i_year-1) & (dates_month == 12)))
        elif season == 'mam':
            time_ind = (((dates_year == i_year) & (dates_month == 3)) | ((dates_year == i_year) & (dates_month == 4))
                        | ((dates_year == i_year) & (dates_month == 5)))
        elif season == 'jja':
            time_ind = (((dates_year == i_year) & (dates_month == 6)) | ((dates_year == i_year) & (dates_month == 7))
                        | ((dates_year == i_year) & (dates_month == 8)))
        elif season == 'son':
            time_ind = (((dates_year == i_year) & (dates_month == 9)) | ((dates_year == i_year) & (dates_month == 10))
                        | ((dates_year == i_year) & (dates_month == 11)))
...
def obs_std_dev(obs_data_file, obs_topo_file):
    """
        Data input should be in the format (time, lat, lon)
        We will calculate the std_dev for the given time_ind, the time_ind has to be a logical array
         of the size of the time dimension
    """
