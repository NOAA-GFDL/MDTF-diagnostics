...
#For legend purposes, add in last CMIP6 model seperately
...
ax1.scatter(np.arange(CMIP6vals.shape[1]-1),np.asarray([Net_RA_Model,LWC_Model+SWC_Model, \
  PL_Model+LR_Model+LWWV_Model+SWWV_Model+Alb_Model]),c='b',label='Your Model')
#Add in Observational Values
ax1.scatter(np.arange(CMIP6vals.shape[1]-1),np.asarray([Net_RA_Obs, \
  LWC_Obs+SWC_Obs,PL_Obs+LR_Obs+LWWV_Obs+SWWV_Obs+Alb_Obs]),c='r',label='Obs.')
