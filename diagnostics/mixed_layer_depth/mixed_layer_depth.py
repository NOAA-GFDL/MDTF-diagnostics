...
def readinocndata(file, varname="so", firstyr="2010", lastyr="2014"):
    ...
    if "latitude" in ds:
        ds = ds.where(
            ds.latitude > 30.0, drop=True
        )  # limit to arctic for now, remove later
    elif "lat" in ds:  # hacky, fix this
        ds = ds.where(ds.lat > 30.0, drop=True)  # limit to arctic for now, remove later
    field = ds[varname]
    field.name = varname
...
print("---")
print("{WK_DIR}/model/")
input_file_so = os.environ["DATADIR"]+"/mon/"+os.environ["CASENAME"]+"."+os.environ["so_var"]+".mon.nc"
input_file_thetao = os.environ["DATADIR"]+"/mon/"+os.environ["CASENAME"]+"."+os.environ["thetao_var"]+".mon.nc"
...
print("figures dir")
print(figures_dir)
obs_file = "{DATADIR}/../../obs_data/mixed_layer_depth/mld_computed_obs_EN4_1979-2014.nc".format(
    **os.environ
)

proc_obs_file = "{DATADIR}/../../obs_data/mixed_layer_depth/EN4_mld_stats.nc".format(
    **os.environ
)
...
monthlyplot(
    modmean,
    figfile=figures_dir + "modelmldmean.png",
    cmapname="viridis",
    myname=modelname,
)
monthlyplot(
    obsmean,
    figfile=figures_dir_o + "obsmldmean.png",
    cmapname="viridis",
