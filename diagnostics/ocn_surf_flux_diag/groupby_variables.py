...
def corr_by_bin(ds_bin_bias,ds,bin1_var,bin2_var,target_var,bin1=10,bin2=10,bin1_range=None,bin2_range=None):
    """
    The function is written to correct the target_var based on the bias shown 
    in binned variable (target_var) in ds_bin_bias. The two variables 
    (bin1_var, bin2_var) as the binning coordinate will be used as the reference
    for bias correction in target_var.
    
    Input:
    - ds_bin_bias (xr.Dataset) : the dataset includes the binned result in terms of bias ratio
    - ds (xr.Dataset)   : the dataset includes all needed variables 
    - bin1_var (string) : variable name in the dataset used as the first
                          binning dimension
    - bin2_var (string) : variable name in the dataset used as the second
    ...
    Output:
    - ds_corr (xr.Dataset) : the dataset includes the corrected target_var ('xxx_corr')
    ...
    for tvar in target_var:
        ...
        for nbin1 in range(bin1):
            for nbin2 in range(bin2):
                if ~np.isnan(ds_bin_bias[tvar][nbin1,nbin2].values):
                    corr_ratio = (1.-ds_bin_bias[tvar][nbin1,nbin2].values)
                else :
                    corr_ratio = 1.
#                 print(corr_ratio)
                da_temp = ds[tvar]\
                ...
                da_temp2 = da_temp.where(da_temp==0.,other=1.)*corr_ratio

                if count == 0:
                    da_out = da_temp.copy()
                    da_out2 = da_temp2.copy()
                else:
                    da_out += da_temp
                    da_out2 += da_temp2
                count += 1
                    
#         da_mask = ds[tvar].where(ds[tvar]!=0.,other)
#         da_mask = da_mask.where(da_mask.notnull(), other=1.)
        
#         da_out = da_mask*ds[tvar]+da_out
#         da_out2 = da_mask*ds[tvar]/ds[tvar]+da_out2
#         da_out = da_out+ds[tvar]
# #         da_out2 = da_out2+ds[tvar]/ds[tvar]

        da_mask = 1-da_out.where(da_out==0.,other=1)   
        da_out = da_out+da_mask*ds[tvar]
        da_out2 = da_out2+da_mask*ds[tvar]/ds[tvar]
        
    
        ds_corr['%s_corr'%tvar]=da_out
        ds_corr['%s_corr_ratio'%tvar]=da_out2
