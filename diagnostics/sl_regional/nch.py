...
def compute_error(threshold, destgrid, wts, conserv_mask, model_flat, cnes_flat, dtu_flat, num_datasets=3):
    ...
    for i in (range(len(destgrid.lon.values))):
        ...
        for j in range(len(destgrid.lat.values)):
            ...
            # if ~np.isnan(cellwt).any() and (cellwt>0).any():
                ...
                # print(i,j,len(ind))
                # Require TCH cells to be sufficiently large in order to run calculation
                if len(ind)> threshold:
                    # print(i,j,len(ind))
                    # Create new array of only the mdt points actually in the cell
                    mdt_tch = np.zeros((3,len(ind)))
                    mdt_tch[0] = cnes_flat[ind]
                    mdt_tch[1] = dtu_flat[ind]
                    ...
                    for k in np.arange(num_datasets):
                        ...
                        # TODO: can you take out this section because anyhwere that Rkk<=0 will be taken care of by if statement in TCH?
                            ...
                            err[k, i, j] = np.nan
                            print(i,j,len(ind),mdt_tch,'tch gives nan')
    ...
    return R, err, num_points, cost_func
