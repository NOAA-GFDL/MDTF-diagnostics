...
#
# Note this is hourly data. The raw files must be averaged into daily-means for this code
# to work properly.
#
...
# spatially degraded by selecting every 8th latitude (so a nominal resolution of 2deg 
# rather than ERA5's default of 0.25 degree). I set the location of these files 
# as environment variables, but explicitly show the paths in comments as examples 
...
# By the end of this block of code, the uwnd and zg variables
# should each contain all available days of zonal-mean zonal winds, and 
# geopotential heights, respectively. They can be
...
out_ds = xr.merge([uwnd_zm, zg_zm, zg_500])
out_ds.attrs['reanalysis'] = 'ERA5'
out_ds.attrs['notes'] = 'Fields derived from daily-mean ERA5 data on pressure levels'
...
# To reduce size of output file even more, we will use zlib compression
# on each variable. This step is also optional.
encoding = {'uwnd_zm':     {'dtype':'float32', 'zlib':True, 'complevel':7},
            'zg_zm':     {'dtype':'float32', 'zlib':True, 'complevel':7},
            'zg_500':     {'dtype':'float32', 'zlib':True, 'complevel':7}}
