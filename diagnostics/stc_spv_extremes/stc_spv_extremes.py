...
def plot_spv_hist(uzm_10, hemi, filepath_ssw, filepath_vi):
    ...
    if (hemi == 'NH'):
        ...
        if ssw:
            ...
            ci_snov_lo, ci_snov_up = sm.stats.proportion_confint(len([i for i in ssw if i.split("-")[1]=='11']), len(ssw), alpha=0.05, method='wilson')
            ci_sdec_lo, ci_sdec_up = sm.stats.proportion_confint(len([i for i in ssw if i.split("-")[1]=='12']), len(ssw), alpha=0.05, method='wilson')
            ci_sjan_lo, ci_sjan_up = sm.stats.proportion_confint(len([i for i in ssw if i.split("-")[1]=='01']), len(ssw), alpha=0.05, method='wilson')
            ci_sfeb_lo, ci_sfeb_up = sm.stats.proportion_confint(len([i for i in ssw if i.split("-")[1]=='02']), len(ssw), alpha=0.05, method='wilson')
            ci_smar_lo, ci_smar_up = sm.stats.proportion_confint(len([i for i in ssw if i.split("-")[1]=='03']), len(ssw), alpha=0.05, method='wilson')

            ci_ssw = [(seas_ssw[0]-ci_snov_lo, ci_snov_up-seas_ssw[0]), (seas_ssw[1]-ci_sdec_lo, ci_sdec_up-seas_ssw[1]), 
                      (seas_ssw[2]-ci_sjan_lo, ci_sjan_up-seas_ssw[2]), (seas_ssw[3]-ci_sfeb_lo, ci_sfeb_up-seas_ssw[3]),
        ...
        if vi:
            ...
            ci_vnov_lo, ci_vnov_up = sm.stats.proportion_confint(len([i for i in vi if i.split("-")[1]=='11']), len(vi), alpha=0.05, method='wilson')
            ci_vdec_lo, ci_vdec_up = sm.stats.proportion_confint(len([i for i in vi if i.split("-")[1]=='12']), len(vi), alpha=0.05, method='wilson')
            ci_vjan_lo, ci_vjan_up = sm.stats.proportion_confint(len([i for i in vi if i.split("-")[1]=='01']), len(vi), alpha=0.05, method='wilson')
            ci_vfeb_lo, ci_vfeb_up = sm.stats.proportion_confint(len([i for i in vi if i.split("-")[1]=='02']), len(vi), alpha=0.05, method='wilson')
            ci_vmar_lo, ci_vmar_up = sm.stats.proportion_confint(len([i for i in vi if i.split("-")[1]=='03']), len(vi), alpha=0.05, method='wilson')
    ...
    if (hemi == 'SH'):
        ...
        if ssw:
            ...
            ci_sjun_lo, ci_sjun_up = sm.stats.proportion_confint(len([i for i in ssw if i.split("-")[1]=='06']), len(ssw), alpha=0.05, method='wilson')
            ci_sjul_lo, ci_sjul_up = sm.stats.proportion_confint(len([i for i in ssw if i.split("-")[1]=='07']), len(ssw), alpha=0.05, method='wilson')
            ci_saug_lo, ci_saug_up = sm.stats.proportion_confint(len([i for i in ssw if i.split("-")[1]=='08']), len(ssw), alpha=0.05, method='wilson')
            ci_ssep_lo, ci_ssep_up = sm.stats.proportion_confint(len([i for i in ssw if i.split("-")[1]=='09']), len(ssw), alpha=0.05, method='wilson')
            ci_soct_lo, ci_soct_up = sm.stats.proportion_confint(len([i for i in ssw if i.split("-")[1]=='10']), len(ssw), alpha=0.05, method='wilson')
            ci_ssw = [(seas_ssw[0]-ci_sjun_lo, ci_sjun_up-seas_ssw[0]), (seas_ssw[1]-ci_sjul_lo, ci_sjul_up-seas_ssw[1]), 
                      (seas_ssw[2]-ci_saug_lo, ci_saug_up-seas_ssw[2]), (seas_ssw[3]-ci_ssep_lo, ci_ssep_up-seas_ssw[3]),
        ...
        if vi:
            ...
            ci_vjun_lo, ci_vjun_up = sm.stats.proportion_confint(len([i for i in vi if i.split("-")[1]=='06']), len(vi), alpha=0.05, method='wilson')
            ci_vjul_lo, ci_vjul_up = sm.stats.proportion_confint(len([i for i in vi if i.split("-")[1]=='07']), len(vi), alpha=0.05, method='wilson')
            ci_vaug_lo, ci_vaug_up = sm.stats.proportion_confint(len([i for i in vi if i.split("-")[1]=='08']), len(vi), alpha=0.05, method='wilson')
            ci_vsep_lo, ci_vsep_up = sm.stats.proportion_confint(len([i for i in vi if i.split("-")[1]=='09']), len(vi), alpha=0.05, method='wilson')
            ci_voct_lo, ci_voct_up = sm.stats.proportion_confint(len([i for i in vi if i.split("-")[1]=='10']), len(vi), alpha=0.05, method='wilson')
