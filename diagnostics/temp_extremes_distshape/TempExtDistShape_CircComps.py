...
#
#   Composites the seasonal circulation patterns associated with days in Non-Gaussian distribution tails at
#   specified locations, following Loikith and Neelin (2019), Loikith et al. (2018), Loikith and Neelin (2015),
#   Ruff and Neelin (2012)
#
#   Generates spatial plot of seasonal composites at lag times to days identified as function of two-meter temperature,
#   sea level pressure, and 500hPa geopotential height
#
#   Depends on the following scripts:
#    (1) TempExtDistShape_CircComps_usp.py
#    (2) TempExtDistShape_CircComps_util.py
#
#   Defaults for location, plotting parameters,  etc. that can be altered by user are in
#   TempExtDistShape_CircComps_usp.py
#   Defaults for season, tail percentile threshold, range of years, etc. that can be altered by user, are in
...
with open(os.environ["WORK_DIR"] + "/TempExtDistShape_CircComps_parameters.json") as outfile:
...
T2Mfile = \
sorted(glob.glob(circ_data["MODEL_OUTPUT_DIR"] + "/" + circ_data["MODEL"] + "*" + circ_data["T2M_VAR"] + ".day.nc"))[0]
SLPfile = \
sorted(glob.glob(circ_data["MODEL_OUTPUT_DIR"] + "/" + circ_data["MODEL"] + "*" + circ_data["SLP_VAR"] + ".day.nc"))[0]
Z500file = \
sorted(glob.glob(circ_data["MODEL_OUTPUT_DIR"] + "/" + circ_data["MODEL"] + "*" + circ_data["Z500_VAR"] + ".day.nc"))[0]

# ======================================================================
# Calculate seasonal subset for each variable and rename from original file
T2M_data, lon, lat, datearrstr, T2M_units = Seasonal_Subset(T2Mfile, circ_data["LON_VAR"], circ_data["LAT_VAR"],
                                                            circ_data["T2M_VAR"], circ_data["TIME_VAR"], monthsub,
                                                            circ_data["yearbeg"], circ_data["yearend"])
T2M_VAR = 'T2M'
SLP_data, lon, lat, datearrstr, SLP_units = Seasonal_Subset(SLPfile, circ_data["LON_VAR"], circ_data["LAT_VAR"],
                                                            circ_data["SLP_VAR"], circ_data["TIME_VAR"], monthsub,
                                                            circ_data["yearbeg"], circ_data["yearend"])
SLP_VAR = 'SLP'
Z500_data, lon, lat, datearrstr, Z500_units = Seasonal_Subset(Z500file, circ_data["LON_VAR"], circ_data["LAT_VAR"],
                                                              circ_data["Z500_VAR"], circ_data["TIME_VAR"], monthsub,
                                                              circ_data["yearbeg"], circ_data["yearend"])
Z500_VAR = 'Z500'
...
(tail_days_lags, statlonind, statlatind, T2M_data, SLP_data, Z500_data, Z500_units, SLP_units,
 T2M_units) = Circ_Comp_Lags(
    T2Manom_data, T2M_data, T2M_units, SLP_data, SLP_units, Z500_data, Z500_units, lat, lon, circ_data["ptile"],
    circ_data["statlat"], circ_data["statlon"], circ_data["lagtot"], circ_data["lagstep"])
...
fig, axes = mplt.subplots(len(numpy.arange(0, circ_data["lagtot"] + circ_data["lagstep"], circ_data["lagstep"])),
                          3,
                          sharex='all', figsize=(30, 15),
                          subplot_kw=dict(projection=ccrs.PlateCarree(central_longitude=lon[statlonind])))
subplotnum = 1
for lag in numpy.arange(0, circ_data["lagtot"] + circ_data["lagstep"], circ_data["lagstep"]):
    ...
    col1 = Plot_Circ_Comp_Lags(T2Mfile, circ_data["LON_VAR"], figstep, 0, lon, lat, newtailinds, T2M_data, T2Manom_data,
                               T2M_VAR, circ_data["Tminval"], circ_data["Tmaxval"], circ_data["Trangestep"], lag,
                               circ_data["Tanomminval"], circ_data["Tanommaxval"], circ_data["Tanomrangestep"],
                               statlonind, statlatind, axes, fig)
    col2 = Plot_Circ_Comp_Lags(SLPfile, circ_data["LON_VAR"], figstep, 1, lon, lat, newtailinds, SLP_data,
                               0, SLP_VAR,
                               circ_data["SLPminval"], circ_data["SLPmaxval"], circ_data["SLPrangestep"], lag,
                               0, 0, 0,
                               statlonind, statlatind, axes, fig)
    col3 = Plot_Circ_Comp_Lags(Z500file, circ_data["LON_VAR"], figstep, 2, lon, lat, newtailinds, Z500_data,
                               Z500anom_data, Z500_VAR, circ_data["Z500minval"], circ_data["Z500maxval"],
                               circ_data["Z500rangestep"], lag, circ_data["Z500anomminval"],
                               circ_data["Z500anommaxval"], circ_data["Z500anomrangestep"], statlonind, statlatind,
                               axes, fig)

# Format colorbars
Set_Colorbars(circ_data["Tminval"], circ_data["Tmaxval"], circ_data["Tcbarstep"], col1, circ_data["lagtot"] // 2,
              0,
              T2M_units, axes, fig)
Set_Colorbars(circ_data["SLPminval"], circ_data["SLPmaxval"], circ_data["SLPcbarstep"], col2, circ_data["lagtot"] // 2,
              1, SLP_units, axes, fig)
Set_Colorbars(circ_data["Z500minval"], circ_data["Z500maxval"], circ_data["Z500cbarstep"], col3,
              circ_data["lagtot"] // 2, 2, Z500_units, axes, fig)
