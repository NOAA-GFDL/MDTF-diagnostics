...
# ----  This is in the /var_code/TempExtDistShape folder under TempExtDistShape_CircComps_usp.py and TempExtDistShape_SeasonAndTail_usp.py
...
os.system("python "+os.environ["VARCODE"]+"/temp_extremes_distshape/"+"TempExtDistShape_CircComps_usp.py")
with open(os.environ["VARCODE"]+"/temp_extremes_distshape/"+"TempExtDistShape_CircComps_parameters.json") as outfile:
...
T2M_data,lon,lat,datearrstr,T2M_units=Seasonal_Subset(T2Mfile,circ_data["LON_VAR"],circ_data["LAT_VAR"],circ_data["T2M_VAR"],circ_data["TIME_VAR"],season_data["monthsub"],season_data["yearbeg"],season_data["yearend"])
T2M_VAR='T2M'
SLP_data,lon,lat,datearrstr,SLP_units=Seasonal_Subset(SLPfile,circ_data["LON_VAR"],circ_data["LAT_VAR"],circ_data["SLP_VAR"],circ_data["TIME_VAR"],season_data["monthsub"],season_data["yearbeg"],season_data["yearend"])
SLP_VAR='SLP'
Z500_data,lon,lat,datearrstr,Z500_units=Seasonal_Subset(Z500file,circ_data["LON_VAR"],circ_data["LAT_VAR"],circ_data["Z500_VAR"],circ_data["TIME_VAR"],season_data["monthsub"],season_data["yearbeg"],season_data["yearend"])
Z500_VAR='Z500'
...
tail_days_lags,statlonind,statlatind,T2M_data,SLP_data,Z500_data,Z500_units,SLP_units,T2M_units=Circ_Comp_Lags(T2Manom_data,T2M_data,T2M_units,SLP_data,SLP_units,Z500_data,Z500_units,lat,lon,season_data["ptile"],circ_data["statlat"],circ_data["statlon"],circ_data["lagtot"],circ_data["lagstep"])

# ======================================================================
### Plot seasonal circulation at specified location for lag times to days in the non-Gaussian distribution tail
# ----- Saves figure in wkdir/TempExtDistShape/
...
print(("Plotting Circulation Composites..."), end=' ')
...
for lag in numpy.arange(0,circ_data["lagtot"]+circ_data["lagstep"],circ_data["lagstep"]):
    ...
    newtailinds=Lag_Correct(lag,figstep,tail_days_lags,datearrstr,circ_data["lagstep"],season_data["monthsub"])
...
os.popen('cp '+os.environ["VARDATA"]+'temp_extremes_distshape/'+circ_data["FIG_OBS_FILENAME"]+' '+circ_data["FIG_OBS_DIR"]+'/')
