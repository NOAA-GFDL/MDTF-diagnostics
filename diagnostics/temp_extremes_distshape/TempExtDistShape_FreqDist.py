...
#
#   Computes the Gaussian distribution fit to the histogram of seasonal two-meter temperature anomalies at
#   specified locations following Loikith and Neelin (2019), Loikith et al. (2018), Loikith and Neelin (2015),
#   Ruff and Neelin (2012)
#
#   Generates plot of the distribution and associated Gaussian fit over season specified as function of
...
with open(os.environ["WORK_DIR"] + "/TempExtDistShape_FreqDist_parameters.json") as outfile:
...
T2Mfile =(
    sorted(glob.glob(freq_data["MODEL_OUTPUT_DIR"] + "/" + freq_data["MODEL"] + "*" + freq_data["T2M_VAR"] + ".day.nc")))[0]

# ======================================================================
# Estimate and plot Gaussian fit to two-meter temperature distribution at specified locations,
# save figure in wkdir/TempExtDistShape/
...
for statind in numpy.arange(0, len(freq_data["citynames"])):
    ...
    Tanom_data, lon, lat = Seasonal_Anomalies(T2Mfile, freq_data["LON_VAR"], freq_data["LAT_VAR"], freq_data["T2M_VAR"],
                                              freq_data["TIME_VAR"], freq_data["monthsub"], freq_data["yearbeg"],
                                              freq_data["yearend"], statind)

    # ======================================================================
    # Estimate Gaussian fit to two-meter temperature distribution at specified location
    bin_centers_gauss, bin_centers, bin_counts, gauss_fit, Tanom_stat = Gaussfit_Est(Tanom_data, lat, lon, statind,
                                                                                     freq_data["statlats"],
                                                                                     freq_data["statlons"],
                                                                                     freq_data["citynames"],
                                                                                     freq_data["binwidth"])
    ...
    # ======================================================================
    # Plot two-meter temperature distribution and estimated Gaussian fit computed above at specified location,
    # save to wkdir/TempExtDistShape
    Gaussfit_Plot(fig, bin_centers, bin_counts, bin_centers_gauss, gauss_fit, Tanom_stat, freq_data["ptile"],
                  freq_data["citynames"], freq_data["monthstr"], statind, freq_data["plotrows"], freq_data["plotcols"])
