...
# ======================================================================
# Gaussfit_Est
# Estimates the Gaussian fit to the histogram of the distribution of two-meter temperature anomalies at
# specified station/season
...
# ---------  Output is centers and counts of histogram bins of temperature anomaly data,
# fixed bin centers if gaussian fit is too wide, gaussian fit, and detrended temperature anomaly array for location
def Gaussfit_Est(T2Manom_data, lat, lon, statind, statlats, statlons, citynames, binwidth):
    ...
    # Compute histogram - numpy.histogram outputs bin edges but require bin centers so define edges here
    # accordingly to match count between hist and bin_edges
    xx, bin_centers = numpy.histogram(Tanom_stat, bins=numpy.arange(min(Tanom_stat) + numpy.true_divide(binwidth, 2),
                                                                    max(Tanom_stat) + numpy.true_divide(binwidth, 2),
                                                                    binwidth))
    bin_counts, xx = numpy.histogram(Tanom_stat,
                                     bins=numpy.arange(min(Tanom_stat), max(Tanom_stat) + binwidth, binwidth))
    ...
    # If the standard deviation of the fit to the core is greater than the SD of the entire distribution,
    # use SD of entire distribution
...
# -----  bin_centers and bin_counts are centers and counts of histogram bins of temperature anomaly data output from
# Gaussfit_Est function above
# -----  bin_centers_gauss are fixed bin centers if Gaussian fit was too wide, output from Gaussfit_Est function above
# -----  gauss-fit is Gaussian fit to 2-meter temperature anomaly distribution at specified location, output from
# Gaussfit_Est function above
