...
echo "# HACK to use absolute path to module; will revise in next release" >> "$_CONDA_WRAPPER"
echo "export PYTHONPATH=\"\$_mdtf\"" >> "$_CONDA_WRAPPER"
echo "/usr/bin/env python -m src.mdtf \"\$@\"" >> "$_CONDA_WRAPPER"
echo "exit_code=\$?" >> "$_CONDA_WRAPPER"
echo "unset PYTHONPATH" >> "$_CONDA_WRAPPER"
